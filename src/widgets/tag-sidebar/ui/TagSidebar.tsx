...
export function TagSidebar({ tags, className = "" }: TagSidebarProps) {
  ...
  const renderTagNode = (tag: TagNode, depth = 0) => {
    ...
    return (
      <div key={tag.id} className="select-none">
        <div
          className="flex items-center py-2 hover:bg-muted/50 rounded-sm"
      ...
      <Card>
        ...
        <CardContent className="pt-0 max-h-96 overflow-y-auto">
          {Object.entries(tagGroups).map(([taxonomyType, tags]) => (
            <div key={taxonomyType} className="mb-4">
              <button
