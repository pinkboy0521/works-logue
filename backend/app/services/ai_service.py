...
- nameable_score: パターン名を付けられる状態か（抽象化可能か）"""
        ...
        try:
            text = await self._call_vertex_ai(
                prompt, "gemini-2.5-flash-lite", settings.VERTEX_AI_TIMEOUT_FLASH
            )
...
- specificity: 明日から実行できるアクションが具体的に抽出可能か"""
        ...
        text = await self._call_vertex_ai(
            prompt, "gemini-2.5-flash", settings.VERTEX_AI_TIMEOUT_SCORING
        )
        parsed = self._parse_json(text)
...
## 明日から使えるアクション"""
        ...
        text = await self._call_vertex_ai(
            prompt, "gemini-2.5-flash", settings.VERTEX_AI_TIMEOUT_LOUGE
        )
        parsed = self._parse_json(text)
        return LougeData(
  ...
  "cleansed_text": "<全固有名詞を置換済みのテキスト>"
        ...
        try:
            raw = await self._call_vertex_ai(
                prompt, "gemini-2.5-flash", settings.VERTEX_AI_TIMEOUT_CLEANSE
            )
